<<<<<<< HEAD:UrbanTrack/js/MainPage.js
    ...
    const sampleIssues = [
      {
        ...
        description:
          "The streetlight at the corner of Main and 5th has been out for over a week.",
      ...
      },
      {
        ...
        description:
          "Dangerous pothole near the school entrance causing traffic issues.",
    ...
    window.goStep = function (n) {
      if (n > currentStep) {
        ...
        if (currentStep === 1 && !selectedSeverity) {
          showToast("Please select a severity level.", true);
          return;
        }
        if (
          currentStep === 2 &&
        ...
        if (
          ...
          showToast("Please enter the issue location.", true);
          return;
        }
      }
      originalGoStep(n);
...
function initDashboard() {
  ...
  const userIssues = issues.filter(
    (issue) => issue.reportedBy === currentUser.id,
  );

  // Display user info
  const userNameElements = document.querySelectorAll(
    ".user-name, .welcome-user, .dashboard-user-name",
  );
  userNameElements.forEach((el) => {
    el.textContent = currentUser.name;
  });
...
>>>>>>> d71744ac2cc31a3e4eb8fd582d854fe3c6af24fc:google-auth-server/UrbanTrack/js/MainPage.js
});
