...
function setAvatarInitials(initials) {
  ...
  if (!avatar) return;
  const textNode = Array.from(avatar.childNodes).find(
...
function initUserSettings() {
  ...
  document
    .querySelectorAll(".user-av")
    .forEach((a) => (a.textContent = initials.slice(0, 2)));
...
function saveProfile() {
  ...
  if (!fn || !ln) {
    showToast("Please fill in your name fields.", "danger");
    return;
  }
...
function saveToggle(el, name) {
  showToast(
    (el.checked ? "✓ " : "✗ ") + name + (el.checked ? " enabled" : " disabled"),
    el.checked ? "success" : "",
  );
}
