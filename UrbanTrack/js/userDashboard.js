const myIssues = [
  {
    ...
    upvotes: 34,
    voted: false,
    desc: "Deep pothole near the bus stop causing damage to vehicles. About 40cm wide and 15cm deep. Very dangerous at night.",
  },
  {
    id: 2,
    ...
    upvotes: 28,
    voted: true,
    desc: "Streetlight has been out for 2 weeks. Children walk past this area daily. Safety hazard at night.",
  },
  {
    id: 3,
    ...
    upvotes: 21,
    voted: false,
    desc: "Drain completely blocked with debris. During rain the street floods up to the pavement.",
  },
  {
    id: 4,
    ...
    upvotes: 9,
    voted: false,
    desc: "Offensive graffiti appeared overnight on the east-facing wall of the community centre.",
  },
  {
    id: 5,
    title: "Water leak on Harris Road",
    category: "Water Leak",
    location: "Midrand",
    status: "in-progress",
    date: "2026-04-07",
    upvotes: 18,
    voted: true,
    desc: "Burst pipe leaking water onto the road. Large puddle forming. Road becoming slippery.",
  },
  {
    id: 6,
    title: "Overgrown trees blocking signage",
    category: "Overgrown Vegetation",
    location: "Fourways",
    status: "resolved",
    date: "2026-03-28",
    upvotes: 7,
    voted: false,
    desc: "Tree branches grown over the stop sign at the intersection.",
  },
  {
    id: 7,
    title: "Cracked pavement on 5th Ave",
    ...
    upvotes: 11,
    voted: false,
    desc: "Several sections of pavement cracked and uplifted. Trip hazard.",
  },
];
...
const communityIssues = [
  {
    id: 101,
    title: "Missing manhole cover on Jan Smuts",
    category: "Other",
    ...
    upvotes: 41,
    voted: false,
    desc: "Manhole cover missing on Jan Smuts Ave. Extremely dangerous for cyclists and pedestrians.",
  },
  {
    id: 102,
    title: "Speed bumps needed on school road",
    category: "Pothole / Road",
    location: "Midrand",
    status: "pending",
    date: "2026-04-09",
    upvotes: 33,
    voted: false,
    desc: "Vehicles speed past school gates every morning during drop-off. Children at serious risk.",
  },
  {
    id: 103,
    title: "No lighting in park at night",
    category: "Streetlight",
    location: "Melville",
    status: "pending",
    date: "2026-04-11",
    upvotes: 22,
    voted: false,
    desc: "Park has no functioning lights after sunset. Community members feel unsafe.",
  },
];
...
function badge(status) {
  const s = statusMap[status] || { badge: "badge-pending", label: status };
...
function refTag(id) {
  return `<span class="ref-tag">#UT-${String(id).padStart(4, "0")}</span>`;
}
...
function renderMyIssues(filter) {
  ...
  const list =
    curFilter === "all"
      ? myIssues
      : myIssues.filter((i) => i.status === curFilter);
...
function filterMyIssues(f, el) {
  document
    .querySelectorAll(".filter-tabs .ftab")
...
function toggleUpvote(id, pool) {
  ...
  showToast(
    issue.voted
      ? " Upvoted! This helps prioritise the issue."
      : "Upvote removed.",
    issue.voted ? "success" : "",
  );
}
...
function initUserDashboard() {
  const currentUser = JSON.parse(
    localStorage.getItem("urbanTrack_currentUser"),
  );
