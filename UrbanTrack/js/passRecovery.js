...
function sendCode() {
  ...
  setTimeout(function() {
    ...
    document.getElementById('otpSub').innerHTML = 'We sent a 6-digit code to <strong>' + em + '</strong>. Check your inbox.';
    goTo(2);
    startTimer();
...
boxes.forEach(function(box, i) {
  ...
  box.addEventListener('paste', function(e) {
    ...
    for (var j = 0; j < pasted.length; j++) {
      if (boxes[j]) { boxes[j].value = pasted[j]; boxes[j].classList.add('filled'); }
    }
    checkOtp();
...
function checkOtp() {
  var full = true;
  boxes.forEach(function(b) { if (!b.value) full = false; });
...
function checkStr() {
  ...
  var levels = [
    { w: '0%',   c: 'transparent', t: 'Enter a password',  tc: 'rgba(255,255,255,0.3)' },
...
function checkMatch() {
  ...
  if (!p2) { hint.textContent = ''; hint.className = 'hint'; return; }
  if (p1 === p2) {
